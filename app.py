...
selected = option_menu(
    menu_title=None,
    options=["Prediction for Supplement", "Explanatory Data Analysis"],
    icons=["pencil-fill", "bar-chart-fill"],  # https://icons.getbootstrap.com/
    orientation="horizontal",
)

if selected == "Prediction for Supplement":
  with st.container():
      # --- Form Input ---
    with st.form("entry_form", clear_on_submit=True):
            ...
            df['prediction_label'] = new_prediction['prediction_label'][0]
            df['prediction_score'] = new_prediction['prediction_score'][0]
  ...
  with st.container():
    with st.form("my_form", clear_on_submit=True):
            ground_truth = st.selectbox('Actual Operative or Ground Truth (Do this patient need supplement or not)',['','Yes-Supplement Anesthesia Needed','No-Supplement Anesthesia Not Needed'],format_func=lambda x: 'Select an option' if x == '' else x)
            df['ground_truth'] = ground_truth
            prediction_label = df['prediction_label'][0]
            submitted = st.form_submit_button("Submit")
    if submitted:
      input_date = str(day) + "/" + str(month) + "/" +str(year)
      year_month = str(year) + "_" + str(month)
      period = str(year)
      db.insert_data(str(datetime.utcnow()),input_date,period,Patient_Gender,Patient_Age,Pulp_Stone_or_Calcification,
                            Pain_Duration_Days,Percussion_Test,Palpation,EPT_duration_before_anaesthesia_seconds,
                            Dental_History,Curved_Canal,Cold_Test_Pain_Duration_before_anaesthesia_seconds,
                            Cold_test_VAS_Score_Before_anaesthesia,Clinical_Pain_VAS_Score,Mobility,Medical_History,
                            EPT_current_pass,EPT_VAS_before_anaesthesia,periodontal_space,PDL_Ligament_involvement,
...
if selected == "Explanatory Data Analysis":
